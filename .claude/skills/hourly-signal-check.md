...
description: Run hourly during market hours. Reconciles open positions, checks the daily-loss circuit breaker, evaluates the stochastic entry signal (dual %K/%D-cross-20 confirmation, spec §3 2026-08-04 revision) and exit signal (state-aware — see the §4 overbought-hold refinement and §4a trend-intact filter) plus earnings/catalyst avoidance against today's candidate list and open positions, and (only if config live=true) places real orders with a resting ATR stop.
---
