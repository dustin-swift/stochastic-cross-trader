...
5. **Sync state out**: `bash scripts/sync_state.sh push` — commits the updated `data/candidates.json` (and today's log file) to `bot-state` so the hourly-signal-check routine sees it. Do this even on a zero-candidates day (see step 2's second bullet) — do **not** do this if step 2 exited non-zero (blocked run), since there's no new state to persist and pushing the old CSV back is a no-op anyway (`sync_state.sh push` correctly no-ops on an unchanged tree).
