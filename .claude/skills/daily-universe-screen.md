...
observed to lag from a cached snapshot — see step 1 below). This skill's own job is: sync state, run the
...
5. Re-run the screen, this time passing that map, so the final `data/candidates.json` reflects the earnings exclusion:
   ```bash
   echo '<earnings map from step 4>' | python3 scripts/check_universe_screen.py --earnings-input -
   ```
